...
def dfa_mdp_synth(dfa, mdp):
    ...
    while not que.empty():
        ...
        #player 2 states, opponent fills out "missing" propositions
            ...
            for opt in powerset(env_ap):
                ...
                for mdp_succ in mdp.successors(mdp_from):
                    # generate configuration guard for chosen option and potential next mdp state (interpreted over joined_ap)
                    mdp_obs = mdp.nodes[mdp_succ]['ap'][0]
                    config = opt_guard + mdp_obs

                    for dfa_succ in dfa.successors(dfa_from):
                        dfa_guards = deepcopy(dfa.edges[dfa_from, dfa_succ]['guard']) # we are gonna delete parts, so deepcopy for safety
                            ...
                            if joined_ap[i] in dfa_ap:
                                ...
                                for guard in dfa_guards:
                                    if guard[j] != 'X' and guard[j] != config[i]:
                                        dfa_guards.remove(guard)    # if a dfa guard is not matching to the current config, remove it
    ...
    #for node in synth.nodes(data= False):
    #    print(node)
    #for edge in synth.edges(data= False):
    #    print(edge)
...
if __name__ == '__main__':
    ...
    prod = dfa_mdp_synth(nx_dfa, nx_mdp)